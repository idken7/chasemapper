...
def parse_config_file(filename):
    ...
    try:
        ...
        _calls = config.get("aprs", "aprs_callsigns", fallback="").strip()
        if _calls == "":
            chase_config["aprs_callsigns"] = []
        else:
            chase_config["aprs_callsigns"] = [c.strip() for c in _calls.split(",") if c.strip()]
        chase_config["aprs_poll_interval"] = config.getint("aprs", "aprs_poll_interval")
