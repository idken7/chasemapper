...
The map is built on [Cesium](https://cesium.com/platform/cesiumjs/), which is self-hosted rather than loaded from a CDN (so the map keeps working with no internet connectivity, once loaded once). The Docker build downloads it automatically; for a local install, fetch it once with:
```
$ ./scripts/fetch_cesium.sh
```
...
Chasemapper's "Share My Live Location" feature uses the browser's geolocation API, which browsers only expose on secure contexts - that is, over HTTPS, or on `localhost`. This is a browser platform restriction, not something chasemapper can work around. If you want chasers to share their live position from phones/tablets over the internet (rather than just on `localhost` or a trusted LAN), you need to put TLS in front of chasemapper. If the app is opened without a secure context, geolocation calls fail immediately and silently unless you're running a build with the visible-error fix below - watch for that toast/settings-row error rather than assuming the feature is broken.

**Which option to use is a deployment decision, not a chasemapper one - it depends entirely on what you (or whoever runs the network the server sits on) are actually able to do:**

- **You control DNS and can open a port to the server** (e.g. a home server, a VPS, a lab machine your IT team will make exceptions for): use **Option 1 (Caddy)** below. Simplest for end users - they just open a normal URL, no extra app required.
- **The server sits on a locked-down network you don't control** (a university/company lab machine behind NAT with no port-forwarding and no ability to get a public DNS record - a common case) : use **Option 3 (Tailscale)** below. No network changes required on the lab's side at all, at the cost of every chaser needing to install the Tailscale app once.
- **You'd rather avoid the HTTPS question entirely**: have chasers use the mobile app (see [mobile/](mobile/) or the SwiftUI+CarPlay app in [ios-app/](ios-app/)) instead of a phone browser. Native GPS access on iOS/Android isn't gated behind a "secure context" the way browser geolocation is, so it works against a plain-HTTP server with zero extra infrastructure - the tradeoff is chasers need the app installed rather than just a URL.
...
### Option 3: Tailscale (no public DNS or open ports needed)
If the server is on a network you don't control the firewall/DNS for - a typical lab or university machine - [Tailscale](https://tailscale.com/) gets you HTTPS without any of that. Install the Tailscale client on the server and on every chaser's phone/laptop; they join a private encrypted mesh network (a "tailnet") that works over NAT and cellular data with no port-forwarding or public domain required. Then, on the server:

```
tailscale serve --bg https / http://localhost:5001
```

This gives you a trusted `https://<machine-name>.<tailnet-name>.ts.net` URL - point chasers at that instead of the server's LAN IP. Certificate issuance/renewal is handled automatically by Tailscale.

The tradeoff versus Caddy/nginx: every device that needs to reach chasemapper (including chasers just viewing the map, not only those sharing location) has to have the Tailscale app installed and be logged into the same tailnet. For a small, known group of chasers this is usually less friction than getting a lab's network team to open a port - but it is an extra app per device, not just a URL.

`tailscale serve` acts as a reverse proxy, so combine this with `CHASEMAPPER_TRUST_PROXY` below.
